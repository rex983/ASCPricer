...
function QuoteDocument({ quote, creatorName }: { quote: Quote; creatorName?: string | null }) {
  const p = quote.pricing as PriceBreakdown;
  const c = quote.config;
  ...
  return (
    <Document>
      <Page size="LETTER" style={styles.page}>
        ...
        {creatorName && (
          <View style={{ marginTop: 12 }}>
            <Text style={{ fontSize: 9, color: "#333", fontStyle: "italic" }}>
              This quote generated by {creatorName}
            </Text>
          </View>
        )}
...
export async function generateQuotePdf(quote: Quote, creatorName?: string | null): Promise<Buffer> {
  return renderToBuffer(<QuoteDocument quote={quote} creatorName={creatorName} />) as Promise<Buffer>;
}
